...
from hunter.settings import MONGODB
...
class BaseModel(Model):
    ...
    def reconnect_amount():
        return xrange(MONGODB['reconnect_tries'] + 1)

    @classmethod
    @gen.coroutine
    def check_reconnect_tries_and_wait(cls, reconnect_number, func_name):
        if reconnect_number >= MONGODB['reconnect_tries']:
            raise gen.Return(True)
        else:
            timeout = MONGODB['reconnect_timeout']
